...
public class CameraService : MonoBehaviour
{
    ...
    void Update()
    {
        if (m_target != null)
        {
            SceneLoadManager.SetCameraPos(transform.position - 2 * nowCameraOffset);
        }
        else
        {
    ...
    {
        if (m_cameraStatus == CameraStatus.None)
        {
            return;
        }

        if (m_n_cclusionTimer < c_n_cclusionInterval)
        {
            m_n_cclusionTimer += Time.deltaTime;
            return;
        }
        m_n_cclusionTimer = 0;

        CharacterBase l_myPlayer = GameLogic.GetMyPlayerCharacter();
        if (l_myPlayer == null)
        {
            //Debug.LogError("MyPlayer is null,con't OcclusionCulling!");
            return;
        }

        m_hideList.Clear();
        Vector3 l_v3_cameraPosition = transform.position;

        Vector3 m_v3_PlayerPos = l_myPlayer.transform.position;
        float m_n_rayLength = Vector3.Distance(m_v3_PlayerPos, l_v3_cameraPosition);

        FindHideObj(m_v3_PlayerPos, l_v3_cameraPosition - m_v3_PlayerPos, m_n_rayLength);

        FindHideObj(m_v3_PlayerPos + playerRightPos, l_v3_cameraPosition - m_v3_PlayerPos, m_n_rayLength);

        FindHideObj(m_v3_PlayerPos + playerLeftPos, l_v3_cameraPosition - m_v3_PlayerPos, m_n_rayLength);




        foreach (var go in m_hideList)
        {
            //go.SetActive(false);
            bool changed = false;
            Material[] newMaterial = MaterialManager.ChangeMeterial(go, "MapHid", ref changed, changeTex);
            if (changed == true)
            {
                for (int i = 0; i < newMaterial.Length; i++)
                {
                    newMaterial[i].SetFloat("_showTime", Time.timeSinceLevelLoad);
                    newMaterial[i].SetFloat("_speed", 1.5f);
                }
            }

            if (m_showList.Contains(go))
            {
                m_showList.Remove(go);
            }
        }
        foreach (var go in m_showList)
        {
            //go.SetActive(true);
            Material[] oldMaterial = go.GetComponent<Renderer>().materials;
            for (int i = 0; i < oldMaterial.Length; i++)
            {
                oldMaterial[i].SetFloat("_showTime", Time.timeSinceLevelLoad);
                oldMaterial[i].SetFloat("_speed", -1.5f);
            }
            MaterialManager.Reduction(go, 0.5f);
        }
        m_showList.Clear();

        for (int i = 0; i < m_hideList.Count; i++)
        {
            m_showList.Add(m_hideList[i]);
        }
